...
export const parseWord = (wordInput: string) => {
  ...
  inputWord = inputWord.map((letter) => {
    return ALPHABET.has(letter) ? letter : "_";
  });
...
export const extendBannedLetters = (
...
export const get10BestLetters = (
  letters: { [key: string]: number },
  word: string
) => {
  let bestLetters = Object.keys(letters).filter(
